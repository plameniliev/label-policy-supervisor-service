...
- **Pin images by digest.** Use `kbld` (or equivalent) to resolve the controller image to a digest at package-build time, not a mutable tag — otherwise `kapp-controller` can silently redeploy a different image than what was tested. Please note this is not the case with this project as it would require specific scripting based on your local container runtime Docker or Podman. Additionally, **tag immutability** rules can be applied at the registry to reject any push that would overwrite an existing tag's digest.
